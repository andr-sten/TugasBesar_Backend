...
class AntrianController extends Controller
{
    ...
    public function index(Request $request) {
        ...
        if ($user->role === 'admin') {
            // Sesuai logic web controller (FrontEndController), admin melihat aktivitas global
    ...
    /**
     * Delete a specific queue history (Owner only)
     */
    public function destroy($id)
    {
        ...
        if ($antrian->user_id !== $user->id) {
            return response()->json([
                'status' => false,
                'message' => 'Akses ditolak. Anda hanya bisa menghapus riwayat Anda sendiri.'
            ], 403);
        }

        // Optional: only allow deleting finished/cancelled queues
        if (!in_array($antrian->status, ['selesai', 'batal'])) {
            return response()->json([
                'status' => false,
                'message' => 'Hanya antrian yang sudah selesai atau batal yang bisa dihapus.'
            ], 400);
        }
        ...
        return response()->json([
            'status' => true,
            'message' => 'Riwayat antrian berhasil dihapus.'
        ], 200);
    }
}
